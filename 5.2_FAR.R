...
new.off <- matrix(rep(0,30*600), nrow = 600, ncol = 30)
new.bid <- matrix(rep(0,30*600), nrow = 600, ncol = 30)

sub.off <- data_off[,180:204]
sub.bid <- data_bid[,199:223]

index <- as.integer(substr(colnames(sub.off),4,5))-1
index[1] <- 1
index
colnames(sub.off) <- index
colnames(sub.bid) <- index

for(i in 1:length(index)){
  new.off[,index[i]] <- sub.off[,i]
  new.bid[,index[i]] <- sub.bid[,i]
}

for(i in 2:30){
  if(new.off[600, i] == 0){
    new.off[,i] <- new.off[,i-1]
  }
  if(new.bid[1, i] == 0){
    new.bid[,i] <- new.bid[,i-1]
  }
}
